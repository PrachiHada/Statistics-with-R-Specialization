...
Ans: Distributions are left skewed therefore (C) is false
```{r habit-weight-box}
# type your code for the Question 3 here, and Knit
boxplot(weight ~ habit, data = nc, xlab = "Habit", ylab = "Weight")
smoker <- nc %>%
  filter(habit == "smoker") %>%
  summarise(iqr = IQR(weight), med = median(weight), min = min(weight), max = max(weight), range = max-min, mean = mean(weight))
...
ggplot(data = smoker_df, aes(x= weight)) + geom_histogram()
...
ggplot(data = nonsmoker_df, aes(x= weight)) + geom_histogram()
...
#Can also achieve same output by
#by(nc, nc$habit, function(nc) mean(nc$weight))
```
...
Answer: The width of the interval decreases as confidence level decereases.
```{r weeks-ci-90}
...
Ans: The following p-value of 0.1711 is over 0.05, and therefore we cannot reject the null hypothesis, or the mean difference in weights is not statistically significant.
...
inference(y = weeks, x = mature, data = nc, statistic = "mean", type = "ht", null = 0, 
          alternative = "twosided", method = "theoretical")

```
P-value of 0.228 is greater than 0.05 therefore, we fail to reject hull hypotheses and determine that there is no significant difference between the means of younger or mature mothers with regards to pregnancy duration.
