...
The population distribution is right skewed and unimodal, as the median (1,481.5) is less than the mean. The range is is about 600.there is an extreme outlierover 5,000. Looking at the summary of the sample size of 60 I would say the typical size within the sample is the sample mean, 1. To me, this means that this value represents an average living area size that most homes have in Ames.
</div>
```{r describe-sample}
...
samp %>%
  summarise(sample_mean = mean(area), sample_med = median(area), 
            sample_sd = sd(area), sample_iqr = IQR(area),
...
library(oilabs)
#lower <- samp %>%
#  summarise(mean(samp$area) - z_star_95 * (sd(samp$area) / sqrt(n)))
#upper <- samp %>%
#  summarise(mean(area) + z_star_95 * (sd(area) / sqrt(n)))

lo <- mean(samp$area) - z_star_95 * (sd(samp$area) / sqrt(60))
up <- mean(samp$area) + z_star_95 * (sd(samp$area) / sqrt(60))


plot_ci(lo, up, mean(ames$area))
...
```{r perc}
count_yes <- length(which(ci_data$capture_mu == "yes")) 
count_yes <- count_yes/2
count_yes
...
ci_data1 <- data.frame(ci_id = c(1:50, 1:50),
                      ci_bounds = c(ci1$lower, ci1$upper),
                      capture_mu = c(ci1$capture_mu, ci1$capture_mu))

					  
ggplot(data = ci_data1, aes(x = ci_bounds, y = ci_id, 
                           group = ci_id, color = capture_mu)) +
  geom_point(size = 2) + geom_line() +
  geom_vline(xintercept = params$mu, color = "darkgray") # draw vertical line	

count_yes <- length(which(ci_data1$capture_mu == "yes")) 
count_yes <- count_yes/2
count_yes
